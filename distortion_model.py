...
def distortionParameter(types):
    ...
    elif types == 'tangential':
        # 切向畸变参数
        p1 = np.random.random_sample() * 0.1 - 0.05
        p2 = np.random.random_sample() * 0.1 - 0.05
        parameters.extend([p1, p2])
        return parameters
...
def distortionModel(types, xd, yd, W, H, parameter):
    ...
    if (types == 'barrel' or types == 'pincushion'):
        ...
        k = parameter[0]
        r = np.sqrt((xd - W / 2) ** 2 + (yd - H / 2) ** 2)
        theta = np.arctan(r)
        rd = theta * (1 + k * theta ** 2) / r if r != 0 else 1  # 防止除以零
        xu = (xd - W / 2) * rd + W / 2
        yu = (yd - H / 2) * rd + H / 2
        return xu, yu
        ...
        k1, k2 = parameter
        r = np.sqrt((xd - W / 2) ** 2 + (yd - H / 2) ** 2)
        rd = r * (1 + k1 * r ** 2 + k2 * r ** 4) / r if r != 0 else 1  # 防止除以零
        xu = (xd - W / 2) * rd + W / 2
        yu = (yd - H / 2) * rd + H / 2
        return xu, yu
        ...
        p1, p2 = parameter
        r2 = (xd - W / 2) ** 2 + (yd - H / 2) ** 2
        xu = xd + (2 * p1 * (xd - W / 2) * (yd - H / 2) + p2 * (r2 + 2 * (xd - W / 2) ** 2))
        yu = yd + (p1 * (r2 + 2 * (yd - H / 2) ** 2) + 2 * p2 * (xd - W / 2) * (yd - H / 2))
        return xu, yu
